...
function my_block_cgb_block_assets() { // phpcs:ignore
	...
	//VF Get all published pages with the fuction 'get_pages'
    ...
    // get_post_thumbnail_id( int|WP_Post $post = null ) - Retrieve post thumbnail ID.

	foreach ($pages as $key => $page) {
		$page->image = wp_get_attachment_image_src(get_post_thumbnail_id($pages[$key]->ID), 'full')[0];
	  }
